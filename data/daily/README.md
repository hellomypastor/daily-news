...
- `image` 可选。优先选择本页已引用的官方原文、论文或项目页面自带的题图/OpenGraph 图；`sourceUrl` 必须对应 `sources` 中的 URL，并出现在正文对应新闻条目里。发布器会把图片插到首次引用该来源的段落或表格之后，而不是作为页面题图或列表缩略图。必须填写准确 `alt` 和来源说明，禁止使用搜索结果缩略图、图库水印图、头像或无法确认出处的图片。没有合适图片时省略，不要硬凑。
