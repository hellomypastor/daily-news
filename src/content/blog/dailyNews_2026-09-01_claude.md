...
**谨慎偏负，置信度中低。** 本日累计 46 条用户 issue 观察信号，样本仍来自天然偏负面的 issue 仓库；v2.1.252 是可确认的修复发布。证据只能说明本轮可见反馈集中于可靠性、权限、Agent 编排与跨环境一致性，不能代表整体用户口碑。
...
- [Browser 文件上传缺口](https://github.com/anthropics/claude-code/issues/91164) 与 [OTLP exporter 静默不发送](https://github.com/anthropics/claude-code/issues/91165)：分别提供工具能力对照，以及接收端、socket 与历史指标对照。
- [code-review plugin 异步结束](https://github.com/anthropics/claude-code/issues/91167)：来自约 40 个仓库、6 个组织的统一 workflow，失败运行会绿灯结束却未实际评审。
- [SSH startDirectory 被忽略](https://github.com/anthropics/claude-code/issues/91171) 与 [claude ssh 不再连接](https://github.com/anthropics/claude-code/issues/91172)：两条相互独立的复现共同指向远端会话入口问题。
- [Keychain 与明文凭据分叉](https://github.com/anthropics/claude-code/issues/91180)：单机报告显示认证诊断误导及仍有效 refresh token 遗留；不是 Anthropic 安全公告。
- [VS Code model/effort picker 脱节](https://github.com/anthropics/claude-code/issues/91184) 与 [scheduled routine egress 限制](https://github.com/anthropics/claude-code/issues/91186)：均附配置、日志或 transcript 对照。
